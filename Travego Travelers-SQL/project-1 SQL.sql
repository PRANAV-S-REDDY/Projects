...
Passenger_id DOUBLE,
Passenger_name____ VARCHAR(100),
Category VARCHAR(100),
Gender_ VARCHAR(100),
Boarding_City_ VARCHAR(100),
Destination_City___ VARCHAR(100),
Distance___ DOUBLE,
Bus_Type VARCHAR(100)
);

/* INSERT QUERY NO: 1 */
INSERT INTO passenger(Passenger_id, Passenger_name____, Category, Gender_, Boarding_City_, Destination_City___, Distance___, Bus_Type)
VALUES
(
...
select Passenger_name____ from passenger
where Passenger_name____ like ('s%');
...
select distinct a.Passenger_name____, a.Boarding_City_, a.Destination_City___,
...
select a.Passenger_name____, a.Distance___, a.Bus_Type, b.price
from passenger a
inner join
price b
on a.Bus_Type = b.Bus_type___
where a.Passenger_name____ like ('%s') 
...
select a.Passenger_name____, a.Bus_Type, a.Destination_City___ as Boarding_City_, a.Boarding_City_ , a.Distance___, b.price, b.distance
from passenger a
join
price b

on a.Distance___ = b.distance
where a.Passenger_name____ = 'pallavi';



#2. g. 	List the distances from the "Passenger" table which are unique (non-repeated distances) in descending order. 
select Distance___ as 'distinct distances' from passenger order by Distance___ desc;
...
select Passenger_name____, Distance___ as 'distance_travelled',
Distance___* 100/(select sum(Distance___) from passenger) as '% of distance_travelled from total distance'
from passenger;
